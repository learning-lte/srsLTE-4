...
// data is the decoded data, this program does nothing to it
  ...
  // Let UE and main threads assigned to designated cores
  ...
  if (!prog_args.input_file_name) {
    ...
    uint32_t ntrial=0; 
    do {
      // Search for Master Information Block (MIB) that contains
      // DL Bandwidth, Number of Transmit Antenna
      // System Frame Number (SFN)
      // PHICH Configuration
      // Transmit every 40 ms , repeat every 10 ms
      // ret= 0 not found, ret = -1 error
  ...
  // PDSCH (Physical Downlink Shared Channel) contains user data (DL payload).
  if (prog_args.input_file_name) {
  ...
  /* Configure MBSFN (Multicast Broadcase Single Frequency Network) area id and non-MBSFN Region */
  if (prog_args.mbsfn_area_id > -1) {
...
//    if(last_nof_symbols != nof_symbols) {
